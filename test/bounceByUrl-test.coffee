...
config = _id: 'rules', foo: { url: 'localhost:3000'}
...
describe 'freeway', ->
  ...
  describe 'features', ->
    it 'should bounce by url', (done) ->
      process.nextTick ->
        request 'http://localhost:4000/hello', headers: { host: 'foo'}, (e, r, b) ->
          b.should.equal 'world'
          done()
    # it 'should bounce with token', (done) ->
    #   config = 'foo': { ip: '127.0.0.1', port: 3000, token: 'foobar'}
    #   request 'http://localhost:4000/hello', headers: { host: 'foo', 'x-token': 'foobar'}, 
    #     (e, r, b) ->
    #       b.should.equal 'world'
    #       done()
